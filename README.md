...
기능적 요구사항
1. 관리자가 도서를 등록한다.
1. 사용자가 전체 도서 목록을 조회한다. (삭제)
1. 사용자가 도서를 예약한다. (삭제)
1. 사용자가 도서를 예약시에는 결제(pay)되어야 도서가 예약된다.
1. 사용자가 본인이 예약 중인 도서를 대여한다.
1. 사용자가 본인이 대여 중인 도서를 반납한다.
1. 사용자가 본인의 예약을 취소할 수 있다.
...
1. 성능
    1. 사용자는 My Page에서 본인 예약 및 대여 도서의 목록과 상태를 확인할 수 있어야한다 CQRS
    1. 예약 중인 도서의 상태가 변경될 때마다 사용자에게 카톡으로 알림을 줄 수 있어야 한다  Event driven (임현진 TEST)
